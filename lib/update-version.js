 ...
 *
 * @returns {Array} The parsed objects from pr messages text file
 *
 ...
 *
 * @returns {string} The release type (minor, major, patch) or null
*/
function getReleaseType(commits) {
  ...
  commits.forEach(commit => {
    ...
    if (isUndefined(commitReleaseType)) {
      commitReleaseType = analyzeCommit(RELEASE_RULES, commit)
    }
...
async function getLastVersion() {
  let lastVersion = process.env.npm_package_version
  ...
  const { body } = await github
    ...
    .catch(error => {
      console.error('Could not find a latest release: Using version from package.json', lastVersion, error)
  ...
  console.warn('Last version detected: ', lastVersion)
